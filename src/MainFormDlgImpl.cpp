...
MainFormDlgImpl::MainFormDlgImpl(QWidget* pParent, const string& strSession) : QDialog(pParent, Qt::WindowMinMaxButtonsHint), m_settings(strSession), m_nLastKey(0)/*, m_settings("Ciobi", "Mp3Diags_v01")*/ /*, m_nPrevTabIndex(-1), m_bTagEdtWasEntered(false)*/, m_strSession(strSession), m_bShowMaximized(false), m_nScanWidth(0)
{
    s_pGlobalDlg = this;
    setupUi(this);
...
void MainFormDlgImpl::initializeUi()
{
    ...
    if (nWidth > 400 && nHeight > 400)
    {
        ...
        // ttt0 rather set m_bShowMaximized on Windows if size is the same as available screen (and perhaps not only on Windows)
    }
    else
    {
        //QRect r (QApplication::desktop()->availableGeometry());
